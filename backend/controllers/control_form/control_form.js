...
console.log('✅ control_form controller loaded successfully');
...
async function getControlNumberPreview(req, res) {
  try {
...
const { getColumnMappingConfig: loadColumnMappingConfig } = require('../../utils/column_mapping');
const {
...
async function getColumnMappingConfig(req, res) {
  try {
    return res.status(200).json({
      success: true,
      data: loadColumnMappingConfig(),
    });
  } catch (error) {
...
async function getControlFrequencyOptions(req, res) {
  try {
    return res.status(200).json({
      success: true,
...
async function bulkImportRows(req, res) {
  try {
    if (req.user.role !== 'company_co') {
...
async function listControlForms(req, res) {
  try {
...
async function listAssignmentEligible(req, res) {
  try {
...
async function getControlFormsStats(req, res) {
  try {
...
async function updateControlForm(req, res) {
  const { form_id } = req.params;
  const {
...
async function submitDeficiencyResponse(req, res) {
...
async function declareNoFurtherSubmission(req, res) {
  const normalizedFormId = String(req.params.form_id || '').trim();
...
async function uploadDeficiencyResponseAttachments(req, res) {
    const normalizedFormId = String(req.params.form_id || '').trim();
    const responseType = String(req.body?.response_type || '').trim();
    const files = [
...
async function getActiveChangeRequest(req, res) {
  const normalizedFormId = String(req.params.form_id || '').trim();
  if (!normalizedFormId) {
    return res.status(400).json({
      success: false,
...
async function getChangeRequestHistory(req, res) {
  const normalizedFormId = String(req.params.form_id || '').trim();
  if (!normalizedFormId) {
    return res.status(400).json({
      success: false,
...
async function bulkSetDueDate(req, res) {
  try {
...
// Create single RACM
async function createControlForm(req, res) {
  const {
    standard_control_description, sub_process, risk_description,
    whether_fraud_risks_exist, control_objective, ipe_reference,
    nature_of_control, control_frequency,
...
async function replicateControlForm(req, res) {
...
// Delete a RACM
async function deleteControlForm(req, res) {
  const { form_id } = req.params;
...
async function getApproverStatus(req, res) {
  const { form_id } = req.params;
...
async function selfAssignRacm(req, res) {
    const { form_id } = req.params;
    ...
    try {
      ...
      const previousProcessOwner = String(form.control_owner || '').trim().toLowerCase() || null;
      ...
      if (hasValidOwner) {
        ...
        const deficiencyStatus = String(form.deficiency_response_status || '')
          .trim()
          .toLowerCase();
      ...
      } else if (status === 'sent for approval' || status === 'approved') {
        return res.status(400).json({
          success: false,
      ...
      if (hasValidOwner) {
        await logAuditEvent(
          'Transfer Process Owner RACM to Coordinator',
          req.user.email_id,
          form_id,
      ...
      return res.status(200).json({
        success: true,
        message: hasValidOwner
          ? 'Process owner RACM transferred to coordinator successfully'
          : 'RACM self-assigned successfully',
        data: {
          ...
          transferred_from_process_owner: Boolean(hasValidOwner),
          previous_process_owner: previousProcessOwner,
        },
      });
    } catch (error) {
...
async function transferSelfAssignedRacmToProcessOwner(req, res) {
  const { form_id } = req.params;
...
async function checkSamplingExists(req, res) {
  const { form_id } = req.params;
  const client = await pool.connect();
...
async function deleteSampleDocument(req, res) {
...
async function uploadSamplingExcel(req, res) {
  const { form_id } = req.params;
  const files = [
...
module.exports = {
  testRoute,
  getControlNumberPreview,
  getColumnMappingConfig,
  getControlFrequencyOptions,
  bulkImportRows,
  listControlForms,
  listAssignmentEligible,
  getControlFormsStats,
  downloadDocument,
  getControlFormById,
  updateControlForm,
  submitDeficiencyResponse,
  declareNoFurtherSubmission,
  uploadDeficiencyResponseAttachments,
  requestChange,
  ...
  bulkSetDueDate,
  createControlForm,
  replicateControlForm,
  deleteControlForm,
  getApproverStatus,
  selfAssignRacm,
  transferSelfAssignedRacmToProcessOwner,
  uploadUserDocument,
  checkSamplingExists,
  deleteUserDocument,
